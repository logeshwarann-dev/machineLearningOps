...
def preprocess_data(data: pd.DataFrame, sample_size: int = 2000):
    """Preprocess the data by splitting features (X) and target (y), and reduce dataset size to the specified sample_size."""
    # Sample the data to the specified number of rows (e.g., 2000 rows)
    data_sampled = data.sample(n=sample_size, random_state=42)
    
    # Split features and target
    X = data_sampled.drop('cardio', axis=1)
    y = data_sampled['cardio']
    return X, y
...
def main():
    ...
    # Preprocess the data and reduce to 2000 rows
    X, y = preprocess_data(data, sample_size=2000)
